...
fn main() {
    ...
    unsafe {
        ...
        scheduler.spawn_thread(&mut STACKS[0], benchmark_yield, 1).unwrap();
        scheduler.spawn_thread(&mut STACKS[1], benchmark_yield, 1).unwrap();
        ...
        if total_yields > 0 {
            ...
            println!("Yields per second: {:.0}", total_yields as f64 / elapsed.as_secs_f64());
        }
    }
    ...
    println!("-------------------------");
    println!("Thread struct size: {} bytes", std::mem::size_of::<preemptive_mlthreading_rust::Thread>());
    println!("Scheduler overhead: ~{} KB", 
            (std::mem::size_of::<preemptive_mlthreading_rust::Scheduler>() + 1023) / 1024);
    println!("Stack size per thread: {} KB", 32);
    println!("Total for 4 threads: ~{} KB", 
