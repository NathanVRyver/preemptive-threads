...
static mut PROCESS_STATS: [ProcessStats; 4] = [
    ProcessStats::new(); 4
];
...
fn process_0() {
    for i in 0..10 {
        ...
        for _ in 0..5000 {
            unsafe { PROCESS_STATS[0].cpu_time += 1; }
        }
...
fn process_1() {
    for i in 0..8 {
        ...
        // Simulate user work
        for _ in 0..3000 {
            unsafe { PROCESS_STATS[1].cpu_time += 1; }
        }
...
fn process_2() {
    for i in 0..6 {
        ...
        for _ in 0..2000 {
            unsafe { PROCESS_STATS[2].cpu_time += 1; }
        }
...
fn process_3() {
    for i in 0..12 {
        ...
        for _ in 0..1000 {
            unsafe { PROCESS_STATS[3].cpu_time += 1; }
        }
...
fn main() {
    ...
    unsafe {
        ...
        scheduler.spawn_thread(&mut PROCESS_STACKS[0], process_0, 10).unwrap(); // System (highest)
        scheduler.spawn_thread(&mut PROCESS_STACKS[1], process_1, 5).unwrap();  // User app
        scheduler.spawn_thread(&mut PROCESS_STACKS[2], process_2, 3).unwrap();  // Background
        scheduler.spawn_thread(&mut PROCESS_STACKS[3], process_3, 1).unwrap();  // Batch (lowest)
