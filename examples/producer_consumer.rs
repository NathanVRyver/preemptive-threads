...
fn main() {
    ...
    unsafe {
        ...
        scheduler.spawn_thread(&mut PRODUCER_STACK, producer_thread, 1).unwrap();
        scheduler.spawn_thread(&mut CONSUMER_STACK, consumer_thread, 1).unwrap();
    ...
    unsafe { print_number(ITEMS_PRODUCED); }
    print_str(b"\nItems Consumed: ");
    unsafe { print_number(ITEMS_CONSUMED); }
    print_str(b"\n\nExample completed!\n");
