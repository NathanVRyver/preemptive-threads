#[cfg(test)]
mod tests {
    use crate::{error::*, scheduler::*, thread::*};
    use std::{vec, vec::Vec};
    ...
    fn test_priority_scheduling() {
        ...
        let stack1: &'static mut [u8] = unsafe { std::mem::transmute(stacks[0].as_mut_slice()) };
        let _low_priority = scheduler.spawn_thread(stack1, test_thread, 1).unwrap();

        let stack2: &'static mut [u8] = unsafe { std::mem::transmute(stacks[1].as_mut_slice()) };
        let high_priority = scheduler.spawn_thread(stack2, test_thread, 10).unwrap();

        let stack3: &'static mut [u8] = unsafe { std::mem::transmute(stacks[2].as_mut_slice()) };
    ...
    fn test_error_types() {
        assert_eq!(
            ThreadError::MaxThreadsReached.as_str(),
            "Maximum number of threads reached"
        );
        assert_eq!(
            ThreadError::InvalidThreadId.as_str(),
            "Invalid thread ID provided"
        );
        assert_eq!(
            ThreadError::ThreadNotRunnable.as_str(),
            "Thread is not in a runnable state"
        );
        assert_eq!(
            ThreadError::StackOverflow.as_str(),
            "Stack overflow detected"
        );
        assert_eq!(
            ThreadError::SchedulerFull.as_str(),
