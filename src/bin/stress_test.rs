...
fn thread0() { worker_thread(0); }
fn thread1() { worker_thread(1); }
fn thread2() { worker_thread(2); }
fn thread3() { worker_thread(3); }
fn thread4() { worker_thread(4); }
fn thread5() { worker_thread(5); }
fn thread6() { worker_thread(6); }
fn thread7() { worker_thread(7); }
fn thread8() { worker_thread(8); }
fn thread9() { worker_thread(9); }
...
fn main() {
    ...
    unsafe {
        ...
        for i in 0..NUM_THREADS {
            scheduler.spawn_thread(&mut STACKS[i], thread_fns[i], 1).unwrap();
        }
