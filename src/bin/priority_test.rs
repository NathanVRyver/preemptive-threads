...
fn main() {
    ...
    unsafe {
        ...
        scheduler.spawn_thread(&mut STACK1, low_priority_thread, 1).unwrap();
        scheduler.spawn_thread(&mut STACK2, medium_priority_thread, 5).unwrap(); 
        scheduler.spawn_thread(&mut STACK3, high_priority_thread, 10).unwrap();
