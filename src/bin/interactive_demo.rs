...
use preemptive_mlthreading_rust::{scheduler::SCHEDULER, sync::yield_thread, thread::ThreadContext, preemption::Preemption};
...
fn print_colored(msg: &[u8], color: u8) {
    ...
    buffer[pos] = 0x1b; pos += 1;
    buffer[pos] = b'['; pos += 1;
    buffer[pos] = b'3'; pos += 1;
    buffer[pos] = color; pos += 1;
    buffer[pos] = b'm'; pos += 1;
...
fn cpu_intensive_worker(id: u8) {
    ...
    for round in 0..20 {
        ...
        unsafe { THREAD_COUNTERS[id as usize] += 1; }
        
        let mut msg = [0u8; 64];
        let msg_str = b"CPU Thread ";
        let mut pos = 0;
        for &b in msg_str { msg[pos] = b; pos += 1; }
        msg[pos] = b'0' + id; pos += 1;
        let progress_str = b" completed round ";
        for &b in progress_str { msg[pos] = b; pos += 1; }
        if round >= 10 { msg[pos] = b'0' + (round / 10) as u8; pos += 1; }
        msg[pos] = b'0' + (round % 10) as u8; pos += 1;
        msg[pos] = b'\n'; pos += 1;
...
fn cooperative_worker(id: u8) {
    ...
    for i in 0..15 {
        ...
        for &b in iter_str { msg[pos] = b; pos += 1; }
        if i >= 10 { msg[pos] = b'0' + (i / 10) as u8; pos += 1; }
        msg[pos] = b'0' + (i % 10) as u8; pos += 1;
        let done_str = b" (yielding)\n";
        for &b in done_str { msg[pos] = b; pos += 1; }
...
fn priority_worker(id: u8) {
    ...
    for i in 0..10 {
        ...
        let mut msg = [0u8; 64];
        let mut pos = 0;
        for &b in priority.0 { msg[pos] = b; pos += 1; }
        let priority_str = b" priority thread ";
        for &b in priority_str { msg[pos] = b; pos += 1; }
        msg[pos] = b'0' + id; pos += 1;
        let work_str = b" doing work ";
        for &b in work_str { msg[pos] = b; pos += 1; }
        msg[pos] = b'0' + (i % 10) as u8; pos += 1;
        msg[pos] = b'\n'; pos += 1;
...
fn memory_worker(id: u8) {
    ...
    for round in 0..10 {
        ...
        for &b in checksum_str { msg[pos] = b; pos += 1; }
...
fn worker0() { worker_thread(0, 0); }
fn worker1() { worker_thread(1, 0); }
fn worker2() { worker_thread(2, 0); }
fn coop0() { cooperative_worker(0); }
fn coop1() { cooperative_worker(1); }
fn prio0() { priority_worker(0); }
fn prio1() { priority_worker(1); }
fn prio2() { priority_worker(2); }
...
fn main() {
    ...
    unsafe {
        ...
        for phase in 0..4 {
            ...
            let mut pos = 0;
            for &b in phase_str { msg[pos] = b; pos += 1; }
            msg[pos] = b'0' + (phase + 1) as u8; pos += 1;
            let complete_str = b" completed!\n";
            for &b in complete_str { msg[pos] = b; pos += 1; }
        ...
        for i in 0..8 {
            if THREAD_COUNTERS[i] > 0 {
                ...
                let mut pos = 0;
                for &b in thread_str { msg[pos] = b; pos += 1; }
                msg[pos] = b'0' + i as u8; pos += 1;
                let completed_str = b" completed ";
                for &b in completed_str { msg[pos] = b; pos += 1; }
                ...
                for &b in tasks_str { msg[pos] = b; pos += 1; }
