...
fn print_benchmark_results() {
    ...
    let header = b"\n=== BENCHMARK RESULTS ===\n";
    unsafe {
        write(1, header.as_ptr(), header.len());
    }

    let context_msg = b"Context Switch (1000 switches): ";
    unsafe {
        write(1, context_msg.as_ptr(), context_msg.len());
    }
    print_number(unsafe { BENCHMARK_RESULTS[0] });
    let cycles_msg = b" cycles\n";
    unsafe {
        write(1, cycles_msg.as_ptr(), cycles_msg.len());
    }

    let cpu_msg = b"CPU Intensive (100k ops): ";
    unsafe {
        write(1, cpu_msg.as_ptr(), cpu_msg.len());
    }
    ...
    unsafe {
        write(1, mem_msg.as_ptr(), mem_msg.len());
    }
    ...
    unsafe {
        write(1, switch_msg.as_ptr(), switch_msg.len());
    }
    ...
    let end_msg = b"========================\n";
    unsafe {
        write(1, end_msg.as_ptr(), end_msg.len());
    }
}
...
fn benchmark_runner() {
    ...
    unsafe {
        ...
        scheduler
            .spawn_thread(&mut STACKS[0], context_switch_benchmark, 1)
            .unwrap();
        scheduler
            .spawn_thread(&mut STACKS[1], cpu_intensive_benchmark, 1)
            .unwrap();
        scheduler
            .spawn_thread(&mut STACKS[2], memory_benchmark, 1)
            .unwrap();
