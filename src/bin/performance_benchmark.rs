...
fn main() {
    ...
    unsafe {
        ...
        scheduler
            .spawn_thread(&mut STACK1, benchmark_thread, 1)
            .unwrap();
        scheduler
            .spawn_thread(&mut STACK2, benchmark_thread, 1)
            .unwrap();
        ...
        println!(
            "Average time per yield: {:?}",
            elapsed / total_yields as u32
        );
        println!(
            "Context switches per second: {:.0}",
    ...
    unsafe {
        ...
        scheduler
            .spawn_thread(&mut STACK3, cpu_intensive_thread, 1)
            .unwrap();
        scheduler
            .spawn_thread(&mut STACK4, cpu_intensive_thread, 1)
            .unwrap();
